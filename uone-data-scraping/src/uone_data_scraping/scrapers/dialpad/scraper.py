...
def _make_api_call(client, call_config, refresher_func_name=None, attempts=0): # endpoint, params, attempts=0):
    client = client.upper()
    print(f"Retrieving API access token for Dialpad for client: {client}")
    access_token = helpers._get_secret(f"integrations/dialpad/{client}_ACCESS_TOKEN")
    base_url = helpers._get_secret(f"integrations/dialpad/{client}_BASE_URL")
    ...
    print(f"Making '{http_method}' request to {final_url} using: '{list(headers.keys())}' with: {call_config.params}")
    ...
    if results.status_code == 401 and attempts <= 2:
        print(f"Failed initial request because of: {results.text}")
        ...
        helpers._refresh_api_key(refresher_func_name=rfunc, source='nic', client=call_config.client)
        return _make_api_call(call_config=call_config, refresher_func_name=rfunc, attempts=attempts)
