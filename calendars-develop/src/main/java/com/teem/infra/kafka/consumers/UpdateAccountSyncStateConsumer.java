...
@ConditionalOnProperty({
  ...
  ConfigPaths.Tasks.UPDATE_ACCOUNT_SYNC_STATE + ".consumer-enabled"})
@AllArgsConstructor
@Slf4j
public class UpdateAccountSyncStateConsumer {
  private static final String CONF = ConfigPaths.Tasks.UPDATE_ACCOUNT_SYNC_STATE;
  private static final Runner RUNNER = new Runner(
      log, UpdateAccountSyncStateConsumer.class.getSimpleName());

  private final UpdateAccountSyncStateTask task;
  ...
  private void consume(ConsumerRecord<String, UpdateAccountSyncState> record) {
